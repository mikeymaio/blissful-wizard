...
const Header = ({ siteTitle }) => {
  ...
  return (
    <>
      <nav className="navbar" role="navigation" aria-label="main navigation" style={{ position: 'fixed', top: 0, left: 0, right: 0, display: "flex", boxShadow: '0 1px 3px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.24)' }}>
        <div className="navbar-start" style={{ marginLeft: "30px", width: "100%", alignItems: "center", display: "flex", alignItems: 'center' }}>
          <h1>
        ...
        <div className="navbar-end" style={{ marginRight: "30px", display: "flex" }}>
          ...
          <div className="navbar-item">
            <Link aria-label="cart" to="/cart">
              {
                quantity > 0 ?
                  <i data-badge="0" className="fas fa-shopping-bag has-text-dark is-size-5" ><span className="cart-count">{quantity}</span></i>
                  :
